...
# relocate it by declaring ``FORGE_ROOT`` (a project-root-relative path, e.g. ``tools/ai``).
# keystone is always mounted at ``<forge-root>/keystone``. Every generated pointer and hook
# command is templated from this one resolver instead of hard-coding ``_forge/``.
...
def read_keystone_toml(path: Path) -> dict:
    """Read ``_forge/.keystone.toml`` (the integration record) into a nested dict.
...
def _generated_banner() -> str:
    ...
    return f"Generated by {forge_root_name()}/keystone/bin/sync.py. Do not edit this file by hand."
...
`{forge_root_name()}/memory/` at session start", present in context from the start.
...
def _hooks_dir() -> str:
    """``<forge-root>/keystone/hooks`` as a POSIX suffix appended after a vendor's root anchor."""
    return f"{forge_root_name()}/keystone/hooks"
...
# Any hook command pointing into the keystone hooks dir is keystone-managed: sync owns it and may
# rewrite it. Derived from the configured root so a relocated dev layer still matches its own hooks.
def _keystone_hook_marker() -> str:
    return f"{_hooks_dir()}/"
